            ...
            for row in f:
                ...
                for col in row:
                    ...
                    if col == ",":  # Dirt without grass
                        g = Sand(x, y, TILES[','])
                        entities.add(g)
                    if col == "!":  # Dirt with grass above
                        g = Sand(x, y, TILES['!'])
                        entities.add(g)
                    if col == "@":  # Dirt with grass to right
                        g = Sand(x, y, TILES['@'])
                        entities.add(g)
                    if col == "#":  # Dirt with grass below
                        g = Sand(x, y, TILES['#'])
                        entities.add(g)
                    if col == "$":  # Dirt with grass to left
                        g = Sand(x, y, TILES['$'])
                        entities.add(g)
                    if col == "%":  # Dirt with grass to left
                        g = Sand(x, y, TILES['%'])
            ...
            for e in entities:
                if e.rect.top == self.rect.top - 32:
                    ...
                    if e.rect.left == self.rect.left or e.rect.left == self.rect.left+32 or e.rect.left == self.rect.left+64 or e.rect.left == self.rect.left-32:
                        ...
                        InteractableGroup.remove(e)
                        interactable.remove(e)
                elif e.rect.top == self.rect.top and e.rect.left == self.rect.left+32:
                    e.image = TILES[',']
                    InteractableGroup.remove(e)
                    interactable.remove(e)
